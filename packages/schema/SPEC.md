...
| `mcp`      | `{ source:"mcp", connector, tool, args? }`  | host broker (§18)                | reads a GRANTED external tool (§17.1); `connector` MUST resolve to an operator-configured connector (§18); `args` ≤ 8 KB. Host resolves it through the pure-read verb `dashboard.connector.read` (§18), which refuses a non-`readOnly` tool WITHOUT parking |
