...
import { DATA_READ_RPC_ALLOWLIST } from "./data-read.js";
import { resolveBinding } from "./data-read-node.js";

async function withTempStateDir<T>(run: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "boardstate-data-"));
  try {
...
describe("dashboard data binding resolver", () => {
  ...
  it("returns the client-resolution signal for stream and computed bindings", async () => {
    // Neither resolves server-side; both answer with the same marker as rpc so the
    // Control UI knows to resolve them client-side (subscription / sibling derive).
  ...
  it("allowlists the read methods the builtin data widgets bind", () => {
