// Custom-widget `widget.json` manifest load + validation (SPEC §8.1).
//
...
// not a convenience. Hand-written guards mirror the schema validators (no zod).

import { DATA_READ_RPC_ALLOWLIST, normalizeDashboardDataLogicalPath } from "@boardstate/schema";
...
/** Validates a parsed `widget.json` object against the schema (SPEC §8.1). */
...
// The on-disk loaders `resolveWidgetDir` / `loadWidgetManifest` (fs) live in
// `@boardstate/core/node` (./manifest-node.ts) so this module stays browser-safe.
