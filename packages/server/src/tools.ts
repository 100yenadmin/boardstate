// The agent-facing dashboard tools (SPEC §4 surface, `dashboard_*` names). Each tool
// validates its params with a strict typebox schema, stamps agent provenance from
  ...
  Type.Object(
    {
      ...
      method: Type.String({ description: "Allowlisted read method." }),
    },
    { additionalProperties: false },
  ),
  Type.Object(
...
function readOptionalTabLayout(record: Record<string, unknown>): DashboardTabLayout | undefined {
...
function contextOwner(ctx: ToolContext | undefined): string {
...
function actorFromContext(ctx: ToolContext | undefined): DashboardActor {
  const normalized =
    contextOwner(ctx)
...
async function runMutation(params: MutationParams) {
  ...
  return toolJson({ doc: result.doc, workspaceVersion: result.doc.workspaceVersion });
}
  ...
  return [
    ...
    },
    {
      ...
      parameters: Type.Object(
        {
          ...
          layout: Type.Optional(
            Type.Union([Type.Literal("grid"), Type.Literal("full")], {
              description: 'Content layout: "grid" (default) or "full" (single full-bleed widget).',
            }),
          ),
        },
        { additionalProperties: false },
      ...
      execute: async (_toolCallId, rawParams) => {
        ...
        const icon = readOptionalString(record, "icon");
        const hidden = readOptionalBoolean(record, "hidden");
        const layout = readOptionalTabLayout(record);
        return await runMutation({
          ...mutationBase,
          changedTabSlug: slug,
          mutate: (draft) => {
