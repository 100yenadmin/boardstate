...
import type { DashboardWidget, WidgetManifestView } from "@boardstate/core";
import {
  displayWidgetTitle,
  renderCustomWidget,
  renderWidgetBody,
...
describe("dashboard widget cell", () => {
  ...
  it("displayWidgetTitle drops only a trailing (custom) suffix", () => {
  ...
  it("renders a placeholder for custom widgets without a host context", () => {
    const container = renderToContainer(
...
function customContext(
  ...
  return {
    ...
    host: { transport: null, basePath: "", sessionKey: "main" },
    onApprove: vi.fn(),
    onReject: vi.fn(),
    ...overrides,
  };
...
describe("renderCustomWidget (approval-gated dispatch)", () => {
