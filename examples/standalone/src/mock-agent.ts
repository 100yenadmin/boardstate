...
/** Drop a prior run's Insights tab (silent — not a tool call) so the build replays. */
async function ensureFreshInsights(host: InProcessHost): Promise<void> {
  const { doc } = (await host.request("dashboard.workspace.get")) as { doc: WorkspaceDoc };
  if (doc.tabs.some((tab) => tab.slug === INSIGHTS_SLUG)) {
    await host.request("dashboard.tab.delete", { slug: INSIGHTS_SLUG, actor: AGENT_ACTOR });
  }
}
...
export function createMockAgent(host: InProcessHost): ChatAgent {
  return async ({ sessionKey, message }, ctx) => {
    ...
    await ensureFreshInsights(host);
    if (ctx.signal.aborted) {
      return;
    }
